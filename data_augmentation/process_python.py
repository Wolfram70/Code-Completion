...
class processor(object):
    ...
    def RenameAndInsertDeadcode(self, indent=True):
        ...
        fill_mask = pipeline("fill-mask", model=model_GraphCodeBert, tokenizer=tokenizer_GraphCodeBert)
        for v in vnames:
            if v in fname:
                continue
            replaced[v] = f"<mask>"
    ...
    def untokenize(self, indent=True, deadcode=None, replaced=False, cut_ratio=0.0, fix_cut_pos=False):
        ...
        prev_sp = None
        prev_ep = None
        prev_indent = 0 #was 0, thus was giving an indent after the first new line regardless of the indent
        indent_size = -1
